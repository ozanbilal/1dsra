...
| `opensees-parity` | 6 | OpenSees backend parity suite (dedicated real-binary runner required) | Explicit checks now locked from real-binary measurements (including transfer metrics) with deterministic + dt-sensitivity gates; solver-quality constraints enabled from OpenSees diagnostics | Local real-binary parity envelope lock completed; cross-runner/production-binary reconfirmation still required before final sign-off | Medium |
...
- After dedicated release runner confirmation, keep `opensees-parity` envelopes synced with production binary fingerprint and raise confidence tier to Medium-High.
