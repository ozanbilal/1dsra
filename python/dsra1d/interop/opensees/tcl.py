...
def render_tcl(config: ProjectConfig, motion_file: Path, output_dir: Path) -> str:
    ...
    lines: list[str] = []
    lines.append("# Auto-generated by GeoWave")
    ...
    lines.append("# Layer representative stress/strain recorders for hysteresis UI channels")
    lines.append("array unset layerRepElem")
    lines.append("for {set e 1} {$e <= $nElemY} {incr e} {")
    lines.append("    set matTag [lindex $matByElem [expr {$e - 1}]]")
    lines.append("    if {![info exists layerRepElem($matTag)]} {")
    lines.append("        set layerRepElem($matTag) $e")
    lines.append("    }")
    lines.append("}")
    lines.append("foreach matTag [lsort -integer [array names layerRepElem]] {")
