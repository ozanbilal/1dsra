...
def render_tcl(config: ProjectConfig, motion_file: Path, output_dir: Path) -> str:
    ...
    lines: list[str] = []
    lines.append("# Auto-generated by 1DSRA")
    ...
    lines.append("    equalDOF $nodeL($i) $nodeR($i) 1 2 3")
    ...
    lines.append(
        ...
        "$thickness $matTag $fBulk $fMass $hPerm $vPerm 0.0 -$g"
    )
    ...
    lines.append("constraints Transformation")
    lines.append("numberer RCM")
    lines.append("system BandGeneral")
    lines.append("test NormDispIncr 1.0e-6 30 0")
    ...
    lines.append("set ok [analyze $nSteps $dt]")
